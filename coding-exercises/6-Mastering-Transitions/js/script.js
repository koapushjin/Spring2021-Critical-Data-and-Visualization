...
function remove(){
  ...
  elementsForPage.transition().duration(3000).attr("transform", function(d, i){
    return "translate("+ xScale(d.key)+ "," + (h - padding) + ")"
  });
...
function removeAndAdd(){
  ...
  let outDataGroups = exitingElements.select("rect");
   ...
   xAxis = d3.axisBottom(xScale);
   xAxis.tickFormat(d=>{return data.filter(dd=>dd.key==d)[0].name;});
   xAxisGroup.transition().delay(1000).duration(2000).call(xAxis).selectAll("text").attr("font-size", 18);

   yMax = d3.max(data, function(d){return d.value});
   yDomain = [0, yMax+yMax*0.1];
   yScale.domain(yDomain);

   elementsForPage.transition().delay(500).duration(2000).attr("transform", function(d, i){
     return "translate("+ xScale(d.key)+ "," + (h - padding) + ")"
   });
...
document.getElementById("buttonD").addEventListener("click", shuffleData);
...
let colorScaleCircle = d3.scaleLinear().domain([30,70]).range([0,1]);
...
function DONTKNOW(){
  ...
  elementsForPage.select("rect")
   ...
   .attr("fill", function(d,i){
     ...
     return pieceWiseColorScale(colorScaleCircle(d.value))
   })
  ;
...
document.getElementById("buttonF").addEventListener("click", DONTKNOW);
