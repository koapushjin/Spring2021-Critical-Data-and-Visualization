...
function main() {
    ...
    <path class="blade2" d="M311.58,411.76c-37.56-2.07-75.18-.49-112.77-.87-20.32-.2-40.63-.05-60.95.06-2.78,0-5.55.86-8.34,1-1.66.06-4.21-.07-4.81-1.05a6,6,0,0,1,.11-5.3,260.3,260.3,0,0,1,36.59-46.36c11.67-11.67,25.34-20.39,39-29.15,6.53-4.18,13.82-7.32,21-10.28,4.94-2,7.62,2.92,10.24,5.52,16.64,16.48,32.79,33.45,49.28,50.07,5.09,5.13,10.93,9.49,16.1,14.54,5.76,5.63,11.2,11.61,16.65,17.56a4.45,4.45,0,0,1,1,3.58c-.26.88-1.91,1.36-2.94,2C311.75,412.62,311.66,412.19,311.58,411.76Z" transform="translate(-82.21 -286.82)"/>
    <path class="blade7" d="M357.65,582.61l135.9-135.85c1.4,4.46,2.47,7.19,3.11,10,9.21,41,7.53,81.28-7.29,120.86-1.77,4.74-4.16,6.4-9.07,6.33-32.47-.46-64.93-.73-97.4-1-7.48-.06-15,.22-22.46.29A13.78,13.78,0,0,1,357.65,582.61Z" transform="translate(-82.21 -286.82)"/>
    <path class="blade6" d="M285.49,599.48c3.8,0,6-.07,8.22,0,9.3.35,18.6,1,27.91,1.07,11.13,0,22.27-.76,33.41-.71,38.6.2,77.21.59,115.82.94,6.44.06,7.76,2.16,3.93,7.45-9.16,12.62-17.75,25.84-28.33,37.17a178.82,178.82,0,0,1-52.86,39.34c-3.58,1.73-7.17,3.45-10.64,5.36-3.73,2-6.56.93-9.34-1.87q-18.09-18.24-36.3-36.36Q313.87,628.43,290.43,605C289.09,603.62,287.88,602.14,285.49,599.48Z" transform="translate(-82.21 -286.82)"/>
    ...
    let customLogos = viz.append("g").attr("class", "customLogo")
        .html(backlogo)
...
// colors: "#f17a6d", "#fcbe56", "#ffff56", "#c5ff64", "#87dd70", "#61dae7", "#56a0ff", "#b38ddf"
    customLogos.select(".blade1").attr("fill", "#f17a6d").attr("opacity", 1)
    customLogos.select(".blade2").attr("fill", "#fcbe56").attr("opacity", 1)
    customLogos.select(".blade3").attr("fill", "#ffff56").attr("opacity", 1)
    customLogos.select(".blade4").attr("fill", "#c5ff64").attr("opacity", 1)
    customLogos.select(".blade5").attr("fill", "#87dd70").attr("opacity", 1)
    customLogos.select(".blade6").attr("fill", "#61dae7").attr("opacity", 1)
    customLogos.select(".blade7").attr("fill", "#56a0ff").attr("opacity", 1)
    customLogos.select(".blade8").attr("fill", "#b38ddf").attr("opacity", 1)
    customLogos.select(".outside").attr("opacity", 0.8).attr("fill", "#242424").attr("stroke", "#242424").attr("stroke-width","10")
    customLogos.select(".insideCircle").attr("fill", "#f6f4e6")
    ...
    dottedLines.append("line")//horizontal right 1
        ...
        .attr("y2", h/2-20);
    grouptexts.append('text')
        .attr('transform', 'translate(' + (w*1.09) + ',' + (h/2-36) + ')')
        .text('Length:');
    grouptexts.append('text')
        .attr('transform', 'translate(' + w*0.87 + ',' + (h/2+12) + ')')
    ...
    grouptexts.append('text')
        ...
        .text('Stroke Weight:');
    grouptexts.append('text')
        .attr('transform', 'translate(' + w*0.21 + ',' + (h/2.7+37) + ')')
    ...
    grouptexts.append('text')
        ...
        .text('X-Position:');
    grouptexts.append('text')
        .attr('transform', 'translate(' + w*0.09 + ',' + (h/2+52) + ')')
        .text('What Time Is It');

    dottedLines.append("line") // radius
        .attr("x1", 565)
        .attr("x2", 675)
        .attr("y1", 570)
        .attr("y2", 570);
    grouptexts.append('text')
        ...
        .text('Radius: How Many Likes Already');

    dottedLines.append("line") // inner color
        .attr("x1", 565)
        .attr("x2", 566)
        .attr("y1", 568)
        .attr("y2", 720);
    grouptexts.append('text')
        .attr('transform', 'translate(590,715)')
        .text('Center Color: Type of Content');
    grouptexts.append('text')
        .attr('transform', 'translate(300,750)')
        .text('(Dark → Light: Picture / Text / Video / Article / else.)');
